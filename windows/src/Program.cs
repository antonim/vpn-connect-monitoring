    ...
    static class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            using (Mutex mutex = new Mutex(true, MutexName, out createdNew))
            {
                ...
                if (!createdNew)
                {
                    // Приложение уже в трее. Запуск руками — просьба показать
                    // настройки; запуск с --tray просто дубль от автозапуска,
                    // и открывать по нему окно нельзя: механизмов автозапуска
                    // два (ключ Run и папка «Автозагрузка»), и при входе в
                    // систему сработать могут оба.
                    if (!trayOnly)
                        showSignal.Set();
                    return;
                }
